...
async function checkAssets(sdk: Kuzzle) {
  const sierra = await sdk.document.get(
  ...
  expect(
    new Date(sierra._source.groups[0].date).getTime(),
  ).toBeGreaterThanOrEqual(now);

  expect(typeof sierra._source.measures.position.values.position.lon).toBe(
    "number",
  );
  expect(typeof sierra._source.measures.position.values.position.lat).toBe(
    "number",
  );

  const romeo = await sdk.document.get(
    "tenant-asset_tracking-kuzzle",
    "assets",
    "Truck-romeo",
  );
  ...
  expect(
    new Date(romeo._source.groups[0].date).getTime(),
  ).toBeGreaterThanOrEqual(now);
}
