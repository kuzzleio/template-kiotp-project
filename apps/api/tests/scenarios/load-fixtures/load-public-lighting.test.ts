...
async function checkAssets(sdk: Kuzzle) {
  ...
  expect(new Date(streetLamp0001._source.groups[0].date).getTime()).toBeGreaterThanOrEqual(now);

  expect(typeof streetLamp0001._source.measures.brightness.values.lumens).toBe('number');
  expect(typeof streetLamp0001._source.measures.powerConsumption.values.watt).toBe('number');
  ...
  expect(new Date(streetLamp0010._source.groups[0].date).getTime()).toBeGreaterThanOrEqual(now);

  expect(typeof streetLamp0010._source.measures.brightness.values.lumens).toBe('number');
  expect(typeof streetLamp0010._source.measures.powerConsumption.values.watt).toBe('number');
}
