...
async function checkAssets(sdk: Kuzzle) {
  const CE2 = await sdk.document.get('tenant-air_quality-kuzzle', 'assets', 'Room-CE2');
  ...
  expect(new Date(CE2._source.groups[0].date).getTime()).toBeGreaterThanOrEqual(now);
  expect(typeof CE2._source.measures.temperature.values.temperature).toBe('number');

  const CM1 = await sdk.document.get('tenant-air_quality-kuzzle', 'assets', 'Room-CM1');
  ...
  expect(new Date(CM1._source.groups[0].date).getTime()).toBeGreaterThanOrEqual(now);
}
