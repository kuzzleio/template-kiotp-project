...
export class BulkImportService {
  ...
  async parseAndImport(rawContent: string, currentIndex: string, uniqueId: string): Promise<void> {
    this.logs = [];
  ...
  async importAssetsAndDevices(
    ...
    try {
      ...
      for (const [lineNumber, line] of content.entries()) {
        ...
        if (isAsset) {
          createDeviceAndAsset.push(this.createAsset(line, uniqueId, currentIndex));
        }

        if (isDevice) {
          createDeviceAndAsset.push(this.createDevice(line, lineNumber, uniqueId, currentIndex));
        }
  ...
  async createAsset(value: Content, uniqueId: string, currentIndex: string): Promise<void> {
    try {
    ...
    await this.updateMetadata('asset', id, value, metadata, uniqueId, currentIndex);
  }
  ...
  async createDevice(
    ...
    const otherTenant = await this.getExistingDeviceTenant(value, id, uniqueId, currentIndex);
    ...
    await this.updateMetadata('device', id, value, metadata, uniqueId, currentIndex);
  }
  ...
  async linkDeviceToAsset(value: Content, uniqueId: string, currentIndex: string): Promise<void> {
    try {
      await this.app.sdk.query<ApiDeviceLinkAssetRequest, ApiDeviceLinkAssetResult>({
        controller: 'device-manager/devices',
        action: 'linkAsset',
  ...
  async addLog(uniqueId: string, message: string, type: LogType, params = {}): Promise<void> {
    const log = {
      log: {
        message,
        params,
