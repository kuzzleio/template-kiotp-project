...
export class AssetTrackingFixtures extends FixturesGenerator {
  ...
  async loadDigitalTwins() {
    await Promise.all([
      ...
      this.createAsset("Truck", "sierra"),
      this.createAsset("Truck", "romeo"),
    ...
    await this.linkAsset("Truck-sierra", "Loka-LOK2", [
      { device: "position", asset: "position" },
    ]);
  ...
  private async generateAbeewayHistory(
    reference: string,
    road: keyof typeof ROADS,
    days: number,
  ) {
    let battery = 100;
  ...
  private async generateLokaHistory(
    reference: string,
    road: keyof typeof ROADS,
    days: number,
  ) {
  ...
  private async generateEnginkoHistory(
    deviceId: string,
    days: number,
    generator: () => number,
  ) {
